...
def mask_account_card(account_details: str) -> Union[str, None]:
    ...
    if len(account_or_card) == 16:
        return print(f"{pay_system} {get_mask_card_number(account_or_card)}")
    elif len(account_or_card) == 20:
        return print(f"{pay_system} {get_mask_account(account_or_card)}")
    else:
        return print("Проверьте правильность введенного номера")


def get_gate(time_event: str) -> str:
